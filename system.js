...
Hooks.once('init', async function() {
    ...
    Handlebars.registerHelper('neuroshimaDiceTooltip', (modifiedResults, target, skill) => {
        ...
        return new Handlebars.SafeString(
            `<div class="neuroshima-dice-tooltip"><div class="dice-results-grid" style="gap:2px;">${dice}</div>`
            + `<div style="margin-top:4px;font-size:0.85em;"><strong>${game.i18n.localize("NEUROSHIMA.Roll.Target")}:</strong> ${target} &nbsp;`
            + `<strong>${game.i18n.localize("NEUROSHIMA.Roll.SuccessPointsAbbr")}:</strong> ${successCount}</div></div>`
        );
    });
