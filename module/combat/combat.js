...
export class MeleeOpposedChat {
  ...
  static async resolveOpposed(messageId, pending, defenderActor, defenseResult) {
    ...
    const defenderTest = new MeleeWeaponTest({
      actor: defenderActor,
  ...
  static async _fireImmediateOnHitScript(state, hitEntry, isOwnerAttacker) {
    return MeleeActionRunner.fireImmediateOnHitScript(state, hitEntry, isOwnerAttacker);
  }

  /**
   * Parse a damage spec string into an array of wound-type tokens.
   * Delegates to MeleeActionRegistry.parseDamageSpec — the canonical implementation.
   *
   * @param {string|null|undefined} damage
   * @returns {string[]}
   */
  static _parseDamageSpec(damage) {
    return MeleeActionRegistry.parseDamageSpec(damage);
  }
  ...
  static async applyOpposedDamage(messageId) {
    ...
    const { allResults, allWoundIds, totalReduced, allReducedDetails } =
      await DuelDamageEngine.processAll(rd, { attackerActor, defenderActor, weaponItem, CombatHelper });
...
export class MeleeResolution {
  ...
  static async resolveSingleExtraAttack(encounter, attack) {
    ...
    const defenderActor = fromUuidSync(defender.actorUuid);
    const freeTest = new MeleeWeaponTest({
      actor: defenderActor,
